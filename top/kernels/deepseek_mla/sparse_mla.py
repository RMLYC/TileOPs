...
def _sparse_mla_kernel(
    ...
    def _sparse_mla_fwd_func(block_I, threads):
        ...
        if padded_H != heads:
            assert (
                kv_group == 1
            ), "here we solve the heads padding automically, other wise you should handle Q copy and Output copy with your mask (when kv_group == 1, use g_i * padded_H:(g_i+1) * padded_H would be handled automically)"
        ...
        def _sparse_mla_fwd_main(
            ...
            with T.Kernel(
                ...
                b_i, g_i = by, bz
                s_i = (
                    (bx + (KV_stride - 1 if CP0 else 0))
                    if REPLICATE_H == 1
                    else (bx // REPLICATE_H + (KV_stride - 1 if CP0 else 0))
                )
...
def _(
    ...
    *inputs,
    ...
    fake_o = torch.empty(
        [batch, seq_len, heads, dim], device=inputs[0].device, dtype=inputs[0].dtype
    )
    return fake_o
