...
class mha_fwd_kernel(Kernel):
    ...
    def __init__(
        ...
        self.kernel = _mha_fwd_kernel(
            self.batch, self.heads, self.seq_len, self.dim, self.is_causal, self.dtype_str
        )
    ...
    def forward(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor):
        return _mha_fwd_wrapped_kernel(
            self.batch,
            self.heads,
            self.seq_len,
            self.dim,
...
class mha_fwd_wgmma_pipelined_kernel(Kernel):
    ...
    def __init__(
        ...
        self.kernel = _mha_fwd_wgmma_pipelined_kernel(
            self.batch, self.heads, self.seq_len, self.dim, self.is_causal, self.dtype_str
        )
    ...
    def forward(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor):
        return _mha_fwd_wgmma_pipelined_wrapped_kernel(
            self.batch,
            self.heads,
            self.seq_len,
            self.dim,
...
class gqa_fwd_kernel(Kernel):
    ...
    def forward(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor):
        return _gqa_fwd_wrapped_kernel(
            self.batch,
            self.heads,
            self.heads_kv,
            self.seq_len,
...
def _gqa_fwd_wgmma_pipelined_kernel(
    batch, heads, heads_kv, seq_len, dim, is_causal, dtype="float16"
):
    scale = (1.0 / dim) ** 0.5 * 1.44269504  # log2(e)
    assert heads % heads_kv == 0, "heads must be divisible by heads_kv"
    groups = heads // heads_kv
