...
class gqa_bwd(Op):
    ...
    def __init__(
        ...
        self.dispatch_kernel(kernel_map)
        self.prep_kernel = self.kernel_map["gqa_bwd_preprocess_kernel"](
            batch, heads, seq_len, dim, self.dtype
        )
        self.kernel = self.kernel_map["gqa_bwd_kernel"](
            batch, heads, heads_kv, seq_len, dim, is_causal, self.dtype, tune=tune
        )
        if not is_hopper():
            self.post_kernel = self.kernel_map["gqa_bwd_postprocess_kernel"](
                batch, heads, seq_len, dim, self.dtype
            )
    ...
    def default_kernel_map(self):
        return {
            "gqa_bwd_preprocess_kernel": flashattn_bwd_preprocess_kernel,
            "gqa_bwd_kernel": gqa_bwd_wgmma_pipelined_kernel if is_hopper() else gqa_bwd_kernel,
